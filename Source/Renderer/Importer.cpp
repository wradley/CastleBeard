...
static bool GetMeshDataFromFBXMesh(
    ...
    // compress vertices (actually the use indices)
        ...
        std::vector<std::tuple<unsigned int, Renderer::Vertex, unsigned int>> indexVertPairs; // vert index, vert, new index
        ...
        std::sort(indexVertPairs.begin(), indexVertPairs.end(), [](
            const std::tuple<unsigned int, Renderer::Vertex, unsigned int> &a,
            const std::tuple<unsigned int, Renderer::Vertex, unsigned int> &b
            ) {
...
static bool TraverseFBXTree(
    FbxManager *manager, 
    FbxNode *node, 
    Renderer::MeshDataPointers &retMeshDatas,
    bool compressVertices
){
    bool retStatus = true;
    ...
    for (int i = 0; i < node->GetNodeAttributeCount(); ++i) {
        if (node->GetNodeAttributeByIndex(i)->GetAttributeType() == FbxNodeAttribute::EType::eMesh) {
            Renderer::MeshData *mesh = new Renderer::MeshData;
            retStatus = GetMeshDataFromFBXMesh(manager, (FbxMesh*)node->GetNodeAttributeByIndex(i), *mesh, compressVertices);
            retMeshDatas.addMeshData(std::shared_ptr<Renderer::MeshData>(mesh));
        }
    }
...
namespace Renderer
{
    ...
    bool Importer::loadFile(const std::string & fp, bool compressVertices)
    {
        // remove old pointers
    ...
    MeshDataPointers Importer::getMeshDataPointers()
    {

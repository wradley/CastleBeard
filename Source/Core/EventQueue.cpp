...
Core::EventQueue::EventQueue() : _size(0), _front(nullptr), _back(nullptr)
{
    // create a dummy node
    Node *dummy = new Node;
    _front = dummy;
...
unsigned int Core::EventQueue::size() const
{
    return _size.load();
}


void Core::EventQueue::pushBack(const std::shared_ptr<const Event> &value)
{
...
std::shared_ptr<const Core::Event> Core::EventQueue::popFront()
{
    ...
    if (_size.load() > 0) {
        ...
        // save the data
        std::shared_ptr<const Core::Event> data = _front->data;
